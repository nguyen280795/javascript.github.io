...
function dateOfNow() {
    ...
    elementId.innerHTML =
        '<p><input type="text" id="date" name="date-picker" size="15" disabled>' +
...
function setDate() {
    ...
    for (i = positionStart; i > 0; i--) {
        ...
        document.getElementById("day" + (i)).style.color = "#D7DBDD";
    }
    for (j = 1; j <= (42 - positionStop); j++) {
        document.getElementById("day" + (j + positionStop)).innerHTML = j;
        document.getElementById("day" + (j + positionStop)).style.color = "#D7DBDD";
    }
}
//chọn tháng option
